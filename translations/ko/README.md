...
### Token Crusher: 셸 노이즈에 토큰을 태우지 마세요

200개 커밋의 `git log`, 400줄의 `npm install`, 300개가 통과하는 테스트 스위트: 모델은 그 모든 것을 읽고, 당신은 그 모든 것에 비용을 냅니다. Token Crusher는 그 출력을 **모델에 도달하기 전에** 압축합니다: 최대 90% 더 작게, 오류·경고·실패는 항상 보존됩니다.

```
egc run git log        # 같은 명령, 압축된 출력
egc run --raw git log  # 비상구: 전체 출력
egc saved              # 누적 절약량, 토큰 비용 없이 로컬에서 계산
```

설계부터 보수적: 작은 출력은 그대로 통과하고, 실패는 항상 살아남으며, 절약 리포트는 컨텍스트 윈도우를 절대 건드리지 않습니다.
### 요청이 아닌 강제
