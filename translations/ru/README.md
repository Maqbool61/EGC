...
### Token Crusher: хватит сжигать токены на шум консоли

`git log` на 200 коммитов, `npm install` на 400 строк, тестовый прогон с 300 зелеными тестами: модель читает все это, а вы за все это платите. Token Crusher сжимает этот вывод **до того, как он попадет в модель**: до 90% меньше, при этом ошибки, предупреждения и падения всегда сохраняются.

```
egc run git log        # та же команда, сжатый вывод
egc run --raw git log  # запасной выход: полный вывод
egc saved              # накопленная экономия, посчитанная локально с нулевой стоимостью токенов
```

Консервативен по замыслу: маленький вывод проходит нетронутым, падения всегда выживают, а отчет об экономии никогда не попадает в окно контекста.
